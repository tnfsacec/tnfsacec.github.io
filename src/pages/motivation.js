    ...
    async function parseTeam(data){
        ...
        for(let i = 0;i < data['president'].length;++i){
            let set = {
                ...
                'motivation': data['pmo'][i],
                'vicePresident': data['vp'][i],
                'vMotivation': data['vpmo'][i]
            }
            temp.push(set)
        }
        return temp
    ...
    async function parseParliamentary(data){
        ...
        for(let i = 0;i < data['parliamentary'].length;++i){
            let set ={
                ...
                'motivation': data['parliamentarymotivation'][i],
            }
            temp.push(set)
        }
        return temp
    ...
    return(
        <div id={"main"}>
            <article className={"post featured"}>
                {/*team*/}
                <div>
                    <header className={'major'}><h1>正副會長候選人參選動機</h1></header>
                    {team.map((data,num)=>{
                        return(
                            <div>
                                <h2>{num+1}號競選團隊</h2>
                                <h2>{data.president}：</h2>
                                <blockquote style={quoteStyle}>{data['motivation']}</blockquote>
                                <h2>{data.vicePresident}</h2>
                                <blockquote style={quoteStyle}>{data['vMotivation']}</blockquote>
                            </div>
                        )
                    })}
                </div>
                ...
                <hr />
                <div>
                    <header className={'major'}><h1>學生議員(全校選區)參選動機</h1></header>
